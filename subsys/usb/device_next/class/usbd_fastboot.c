...
static struct fastboot_ctx fb_ctx = {0};
static struct usbd_class_data *fb_c_data = NULL;
static const struct fastboot_ops *fb_ops = NULL;
static int iface_is_hs = false;
			 ...
			 const char *data, size_t len)
{
	...
	uint8_t ep_in = iface_is_hs ? fastboot_hs_bulk_in.bEndpointAddress : fastboot_fs_bulk_in.bEndpointAddress;
			    ...
			    struct net_buf *buf, int err)
{
	...
	uint8_t ep_out = iface_is_hs ? fastboot_hs_bulk_out.bEndpointAddress : fastboot_fs_bulk_out.bEndpointAddress;
...
static void fastboot_disable(struct usbd_class_data *const c_data)
{
	struct usbd_context *ctx = usbd_class_get_ctx(c_data);
	uint8_t ep_out = iface_is_hs ? fastboot_hs_bulk_out.bEndpointAddress : fastboot_fs_bulk_out.bEndpointAddress;
	uint8_t ep_in = iface_is_hs ? fastboot_fs_bulk_in.bEndpointAddress : fastboot_fs_bulk_in.bEndpointAddress;
...
{
	STRUCT_SECTION_FOREACH(usbd_fastboot_config, cfg) {
		return cfg;
	}
	return NULL;
}
